...
static inline uint8_t get_uint32(const uint8_t *data, uint32_t *idx, uint32_t *v) {
	MUST(*idx + sizeof(uint32_t) < API_BUFFER_SIZE_BYTES);
	os_memcpy(v, &data[*idx], sizeof(uint32_t));	// copy to avoid unaligned access
	*idx = *idx + sizeof(uint32_t);
	return 1;
}
static inline uint8_t get_uint16(const uint8_t *data, uint32_t *idx, uint16_t *v) {
	MUST(*idx + sizeof(uint16_t) < API_BUFFER_SIZE_BYTES);
	os_memcpy(v, &data[*idx], sizeof(uint16_t));	// copy to avoid unaligned access
...
// --- validate inputs ---
static uint8_t validate_inputs(const uint8_t *data, uint32_t *idx, UTXO_INPUT **inputs_ptr, uint16_t *inputs_count) {
	...
	MUST(*inputs_count >= INPUTS_MIN_COUNT && *inputs_count <= INPUTS_MAX_COUNT);
	...
	*inputs_ptr = (UTXO_INPUT*) &data[*idx];
	...
	for (uint32_t i=0;i<*inputs_count;i++) {
		...
		UTXO_INPUT tmp;
		os_memcpy(&tmp, &data[*idx], sizeof(UTXO_INPUT));	// copy to avoid unaligned access
...
// --- validate outputs ---
static uint8_t validate_outputs(const uint8_t *data, uint32_t *idx, SIG_LOCKED_SINGLE_OUTPUT **outputs_ptr, uint16_t *outputs_count) {
	// uses safe getter macro that returns an error in case of invalid access
	MUST(get_uint16(data, idx, outputs_count));

	// Outputs Count must be 0 < x < 127.
	// At least one output must be specified.
	MUST(*outputs_count >= OUTPUTS_MIN_COUNT && *outputs_count <= OUTPUTS_MAX_COUNT);
	...
	*outputs_ptr = (SIG_LOCKED_SINGLE_OUTPUT*) &data[*idx];
	...
	for (uint32_t i=0;i<*outputs_count;i++) {
		...
		os_memcpy(&tmp, &data[*idx], sizeof(SIG_LOCKED_SINGLE_OUTPUT)); // copy to avoid unaligned access
...
uint8_t validate_payload(const uint8_t *data, uint32_t *idx) {
	...
	MUST(payload_length < API_BUFFER_SIZE_BYTES);			// prevent overflows / wrap-arounds
...
uint8_t validate_inputs_bip32(const uint8_t *data, uint32_t *idx, uint16_t inputs_count, API_INPUT_BIP32_INDEX **inputs_bip32_indices) {
	*inputs_bip32_indices = (API_INPUT_BIP32_INDEX*) &data[*idx];
	...
	for (uint32_t i=0;i<inputs_count;i++) {
		...
		os_memcpy(&tmp, &data[*idx], sizeof(API_INPUT_BIP32_INDEX));	// copy to avoid unaligned access
...
uint8_t validate_count_signature_types(uint32_t idx, const API_INPUT_BIP32_INDEX *inputs_bip32_indices, uint16_t inputs_count, uint8_t *sig_types, uint8_t single_sign) {
	...
	os_memset(sig_types, 0xff, INPUTS_MAX_COUNT);

	// count signature and reference unlock blocks by comparing and marking
	for (uint32_t i=0;i<inputs_count;i++) {
		API_INPUT_BIP32_INDEX tmp_i;
		os_memcpy(&tmp_i, &inputs_bip32_indices[i], sizeof(API_INPUT_BIP32_INDEX));	// copy to avoid unaligned access
		...
		for (uint32_t j=i+1;j<inputs_count;j++) {
			...
			os_memcpy(&tmp_j, &inputs_bip32_indices[j], sizeof(API_INPUT_BIP32_INDEX));	// copy to avoid unaligned access
			...
			// key indices equal?
			if (!os_memcmp(&tmp_i, &tmp_j, sizeof(API_INPUT_BIP32_INDEX))) {
...
static uint8_t validate_inputs_duplicates(const UTXO_INPUT *inputs, uint16_t inputs_count) {
	// Every combination of Transaction ID + Transaction Output Index must be unique in the inputs set.
	for (uint32_t i=0;i<inputs_count;i++) {
		for (uint32_t j=i+1;j<inputs_count;j++) {
			// we can check all bytes because first is the input_type that must always be 0
			if (!os_memcmp(&inputs[i], &inputs[j], sizeof(UTXO_INPUT))) {
				return 0;
			}
		}
	}
...
static uint8_t validate_outputs_duplicates(const SIG_LOCKED_SINGLE_OUTPUT *outputs, uint16_t outputs_count) {
	...
	for (uint32_t i=0;i<outputs_count;i++) {
		for (uint32_t j=i+1;j<outputs_count;j++) {
			...
			if (!os_memcmp(&outputs[i], &outputs[j], sizeof(1 + 1 + ADDRESS_SIZE_BYTES))) {
				return 0;
			}
		}
	}
...
static uint8_t validate_inputs_lexical_order(const UTXO_INPUT *inputs, uint16_t inputs_count) {
	...
	for (uint32_t i=0;i<inputs_count-1;i++) {
		...
		if (memcmp_bytewise((uint8_t*) &inputs[i], (uint8_t*) &inputs[i+1], sizeof(UTXO_INPUT)) != -1) {
			return 0;
		}
	}
	return 1;
...
static uint8_t validate_outputs_lexical_order(const SIG_LOCKED_SINGLE_OUTPUT *outputs, uint16_t outputs_count) {
	// at least 2 needed for check
	if (outputs_count < 2) {
		return 1;
	}
	...
	for (uint32_t i=0;i<outputs_count-1;i++) {
		...
		if (memcmp_bytewise((uint8_t*) &outputs[i], (uint8_t*) &outputs[i+1], sizeof(SIG_LOCKED_SINGLE_OUTPUT)) != -1) {
			return 0;
		}
	}
	return 1;
...
static uint8_t essence_verify_remainder_address(uint32_t *bip32_path, SIG_LOCKED_SINGLE_OUTPUT *outputs, uint32_t outputs_count, uint16_t remainder_index, API_REMAINDER_BIP32_INDEX *remainder_bip32) {
...
//	debug_print_hex((const uint8_t*) outputs[remainder_index].address, 32, 16);
//	debug_print_hex((const uint8_t*) tmp.address, 32, 16);

	// verify, the address is the same
	// relies on packed struct
	MUST(!os_memcmp(&outputs[remainder_index].address_type, &tmp.address_type, ADDRESS_WITH_TYPE_SIZE_BYTES));
...
uint8_t essence_parse_and_validate(API_CTX *api) {
	...
	// parse data
	MUST(validate_inputs(api->data.buffer, &idx, &api->essence.inputs, &api->essence.inputs_count));
	MUST(validate_outputs(api->data.buffer, &idx, &api->essence.outputs, &api->essence.outputs_count));
	...
	MUST(validate_inputs_bip32(api->data.buffer, &idx, api->essence.inputs_count, &api->essence.inputs_bip32_index));
	...
	if (api->essence.has_remainder) {
		MUST(essence_verify_remainder_address(api->bip32_path, api->essence.outputs, api->essence.outputs_count, api->essence.remainder_index, &api->essence.remainder_bip32));
#if 0
	...
	MUST(validate_count_signature_types(idx, api->essence.inputs_bip32_index, api->essence.inputs_count, api->essence.signature_types, api->essence.single_sign_mode));

	// additional validation steps of parsed data
	MUST(validate_inputs_duplicates(api->essence.inputs, api->essence.inputs_count));
	MUST(validate_outputs_duplicates(api->essence.outputs, api->essence.outputs_count));

	MUST(validate_inputs_lexical_order(api->essence.inputs, api->essence.inputs_count));
	MUST(validate_outputs_lexical_order(api->essence.outputs, api->essence.outputs_count));

	return 1;
}


static uint16_t essence_sign_signature_unlock_block(SIGNATURE_UNLOCK_BLOCK* pBlock, uint16_t output_max_len, const uint8_t *essence, uint16_t essence_length,
		uint32_t *bip32_path, API_INPUT_BIP32_INDEX *input_bip32_index) {
	...
	// sign essence
	MUST(ed25519_sign(&pk, essence, essence_length, pBlock->signature, &signature_length));
...
static uint16_t essence_sign_reference_unlock_block(REFERENCE_UNLOCK_BLOCK* pBlock, uint16_t output_max_len, uint8_t signature_type) {
...
static uint16_t essence_sign_single_int(
		uint8_t* output, uint16_t output_max_len,
		uint8_t* essence, uint16_t essence_length,
		uint32_t* bip32_path, uint8_t signature_type, API_INPUT_BIP32_INDEX *input_bip32_index) {


	// if MSB is set, it's a signature unlock block
	if (signature_type & 0x80) {
		return essence_sign_signature_unlock_block((SIGNATURE_UNLOCK_BLOCK*) output, output_max_len, essence, essence_length, bip32_path, input_bip32_index);
	} else {
		return essence_sign_reference_unlock_block((REFERENCE_UNLOCK_BLOCK*) output, output_max_len, signature_type);
	}
}

uint16_t essence_sign_single(API_CTX *api, uint8_t* output, uint16_t output_max_len, uint32_t signature_index) {
	...
	os_memcpy(&input_bip32_index, &api->essence.inputs_bip32_index[signature_index], sizeof(API_INPUT_BIP32_INDEX));	// avoid unaligned access

	uint16_t signature_size = essence_sign_single_int(
			output, output_max_len,
			api->data.buffer, api->essence.length,
...
uint8_t essence_sign(API_CTX *api) {
	...
	for (uint32_t i=0;i<api->essence.inputs_count;i++) {
		...
		API_INPUT_BIP32_INDEX input_bip32_index;
		os_memcpy(&input_bip32_index, &api->essence.inputs_bip32_index[i], sizeof(API_INPUT_BIP32_INDEX));	// avoid unaligned access

		uint16_t signature_size = essence_sign_single_int(output, output_max_len, api->data.buffer, api->essence.length, api->bip32_path, api->essence.signature_types[i],
				&input_bip32_index);
	...
	os_memcpy(api->data.buffer, &api->data.buffer[signature_start_ofs], signature_ofs - signature_start_ofs);
	...
	if (api->data.length < API_BUFFER_SIZE_BYTES &&
		...
		os_memset(&api->data.buffer[api->data.length], 0, API_BUFFER_SIZE_BYTES - api->data.length);
	}
