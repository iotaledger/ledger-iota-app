...
// --- validate outputs ---
static uint8_t
validate_outputs_chrysalis(const uint8_t *data, uint32_t *idx,
...
// --- validate outputs ---
static uint8_t validate_outputs_stardust(const uint8_t *data, uint32_t *idx,
...
static uint8_t validate_outputs_lexical_order_chrysalis(
...
// --- CHECK OUTPUTS FOR LEXICOGRAPHICAL ORDER
static uint8_t
validate_outputs_lexical_order_stardust(const BASIC_OUTPUT *outputs,
                                        uint16_t outputs_count)
{
    // at least 2 needed for check
    if (outputs_count < 2) {
...
static uint8_t essence_verify_remainder_address_chrysalis(
...
static uint8_t essence_verify_remainder_address_stardust(
...
uint8_t essence_parse_and_validate_chryslis(API_CTX *api)
{
    ...
    MUST(validate_outputs_chrysalis(
        api->data.buffer, &idx,
    ...
    if (api->essence.has_remainder) {
        MUST(essence_verify_remainder_address_chrysalis(
    ...
    MUST(validate_inputs_lexical_order(api->essence.inputs,
                                       api->essence.inputs_count));
    MUST(validate_outputs_lexical_order_chrysalis(
        (SIG_LOCKED_SINGLE_OUTPUT *)api->essence.outputs,
        api->essence.outputs_count));
...
uint8_t essence_parse_and_validate_stardust(API_CTX *api)
{
    ...
    MUST(validate_outputs_stardust(api->data.buffer, &idx,
    ...
    // if remainder output, check the address
    if (api->essence.has_remainder) {
        MUST(essence_verify_remainder_address_stardust(
    ...
    MUST(validate_inputs_lexical_order(api->essence.inputs,
                                       api->essence.inputs_count));
    MUST(validate_outputs_lexical_order_stardust(
