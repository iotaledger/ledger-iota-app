...
static unsigned int ux_confirm_output_accept_prev_cb(const bagl_element_t *e) {
	UNUSED(e);
	flow_data.flow_outputs_index_current = flow_data.api->essence.outputs_count-1;
	flow_data.flow_scroll_ypos = 100; // gets changed to the right value in populate_data
...
static unsigned int user_confirm_next_dataset_cb(const bagl_element_t *e) {
	...
	if (flow_data.flow_outputs_index_current >= flow_data.api->essence.outputs_count) {
...
static unsigned int user_confirm_next_cb(const bagl_element_t *e) {
	// the last line of the current datasheet is shown in the middle of the page?
...
static void populate_data() {
	...
	// in user-confirm-mode reorder the datasets in a way that remainder always is the last dateset
	...
	if (flow_data.flow_mode == FLOW_NEW_ADDRESS) {
		...
		if (flow_data.api->essence.has_remainder) {
			...
			if (flow_data.api->essence.remainder_index == flow_data.api->essence.outputs_count-1) {
				// yes, but current index only is the remainder if it's the remainder_index
				// in case of an essence with only one remainder output, this also would be true
				if (read_index == flow_data.api->essence.remainder_index) {
					type = REMAINDER;
				}
			} else {
				// no it's not - we have to take care about switching indices for displaying on the UI
				// current read_index is the last dataset? -> display remainder
				// current read_index is the remainder? -> display last dataset
	...
	address_encode_bech32(&flow_data.api->essence.outputs[read_index].address_type, tmp_bech32, sizeof(tmp_bech32));
	...
	if (type == REMAINDER || type == OUTPUT) {
		...
		if (flow_data.flow_scroll_ypos == 4 /* for both output types the same constant*/ ) {
			reset_toggle = 0;
		}
	}
	...
	// iterate through all lines to display and populate them with the right data
	for (short i=0;i<5;i++) {
		...
		case 0:
			...
			case OUTPUT: {
				...
				// this is safe because the case of an essence with only one remainder address as
				// output is already is covered (is_bip32_remainder flag would be set).
				int non_remainder_outputs = flow_data.api->essence.outputs_count - !!flow_data.api->essence.has_remainder;

				// more than one? Show with numbers on the UI
				if (non_remainder_outputs > 1) {
					snprintf(flow_data.flow_lines[i], sizeof(flow_data.flow_lines[i])-1, "Send To [%d]", flow_data.flow_outputs_index_current+1);
				} else {
		...
		case 4:
			...
			// This is safe because the bech32 string is 64 characters long and the buffer is 65 bytes (incl zero terminator).
			// Additionally, each line of flow_data is 17 characters including it's own zero-terminator.
			// So, 4 parts with 16 bytes each can be safely copied and displayed.
			memcpy(flow_data.flow_lines[i], &tmp_bech32[(cy-1)*LINE_WIDTH], LINE_WIDTH);
			break;
		case 5:
		...
		case 6: {
			...
			os_memcpy(&amount, &flow_data.api->essence.outputs[read_index].amount, sizeof(uint64_t));

			if (flow_data.amount_toggle) {	// full
				// max supply is 2779530283277761 - this fits nicely in one line on the Ledger nano s
				// always cut after the 16th char to not make a page with a single 'i'.
				format_value_full(flow_data.flow_lines[i], sizeof(flow_data.flow_lines[i]), amount);

				// (is done later anyways)
				// ui_output.lines[i][LINE_WIDTH]='\0';
			} else { // short
				format_value_short(flow_data.flow_lines[i], sizeof(flow_data.flow_lines[i]), amount);
			}
			break;
		}
		case 7:
		...
		case 10: // bip32 third line
			format_bip32(flow_data.flow_bip32, cy-8, flow_data.flow_lines[i], sizeof(flow_data.flow_lines[i]));
			break;
		}
...
static void ui_confirm_output_step(short dir) {
	if (dir == -1) {
		...
		flow_data.flow_scroll_ypos = MIN(flow_data.flow_scroll_ypos++, flow_data.number_of_lines-1-2);
	}
	populate_data();
	ux_flow_init(0, ux_flow_confirm_output, &ux_confirm_output_address_step);
}

static void flow_start_int(const API_CTX *api, uint8_t mode, accept_cb_t accept_cb, reject_cb_t reject_cb, timeout_cb_t timeout_cb, const uint32_t bip32[BIP32_PATH_LEN]) {
	...
	flow_data.flow_scroll_ypos = -2;	// data[0] vertically in the middle of the screen
...
void flow_start_user_confirm(const API_CTX *api, accept_cb_t accept_cb, reject_cb_t reject_cb, timeout_cb_t timeout_cb, const uint32_t bip32[BIP32_PATH_LEN]) {
	flow_start_int(api, FLOW_USER_CONFIRM, accept_cb, reject_cb, timeout_cb, bip32);
