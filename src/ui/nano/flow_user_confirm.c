...
static uint8_t get_no_lines_bip32() {
    // at least one line
    uint8_t no_lines = 1;
...
static void populate_data_new_address()
{
    ...
    flow_data.number_of_lines = 6 + get_no_lines_bip32();
...
static void populate_data_outputs()
{
    ...
    switch (type) {
    case REMAINDER:
        flow_data.number_of_lines = 8 + get_no_lines_bip32();
        break;
    case OUTPUT:
...
static void populate_data() {
    ...
    if (flow_data.flow_mode == FLOW_NEW_ADDRESS) {
        populate_data_new_address();
    } else {
        populate_data_outputs();
    }
}
